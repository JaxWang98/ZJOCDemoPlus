...
typedef enum : NSUInteger {
    ...
    EHINavigatorOpenTypeTabbarSelect,
...
+ (void)tabbarSelect:(id)viewController{
    [self tabbarSelect:viewController extraParams:nil animated:YES];
}

+ (void)tabbarSelect:(id)viewController extraParams:(NSDictionary *)extraParams animated:(BOOL)animated{
    [self open:viewController extraParams:extraParams type:EHINavigatorOpenTypeTabbarSelect animated:animated];
}
    ...
    if (controller) {
        ...
        switch (type) {
            ...
            case EHINavigatorOpenTypePresent: {
                ...
                break;
            case EHINavigatorOpenTypeTabbarSelect: {
                
//                [nav pushViewController:controller animated:animated];
                
                controller.hidesBottomBarWhenPushed = NO;
                id<UIApplicationDelegate>  dele = [UIApplication sharedApplication].delegate;
                UITabBarController *tabVC = (UITabBarController *)dele.window.rootViewController;
                ...
                for (UINavigationController *navi in childVC) {
                    if ([navi isKindOfClass:[UINavigationController class]]) {
                        ...
                        if ([controller isKindOfClass:[vcs[0] class]]) {
                            //清除vcs中 除了第一个外其他的vc
                            if (vcs.count > 1) {
                                [vcs removeObjectsInRange:NSMakeRange(1, vcs.count - 1)];
                            }
                            navi.viewControllers = [NSArray arrayWithArray:vcs.copy];
                            [tabVC setSelectedViewController:navi];
                            EHITabbar *tabbar = (EHITabbar *)tabVC.tabBar;
                            [tabbar setCurrentIndex:[childVC indexOfObject:navi]];
                        }
                    }
                }
            }
...
#pragma mark - 获取前面一个控制器
+ (UIViewController *)getPreController {
    NSArray<UIViewController *> *vcs = [self currentNavigationController].viewControllers;
    
    NSUInteger idx = [vcs indexOfObject:[self currentViewController]];
...
+ (NSString *)getPreControllerName {
    UIViewController *vc = [self getPreController];
    if (!vc) {
        return @"";
    } else {
        Class cls = [vc class];
...
+ (UIViewController *)obtainNavigationStackWithViewControllerName:(NSString *)vcName {
    return [self obtainNavigationStackWithViewControllerName:vcName reverse:NO];
}

+ (UIViewController *)obtainNavigationStackWithViewControllerName:(NSString *)vcName reverse:(BOOL)reverse {
    UINavigationController * nav = [self currentNavigationController];
    UIViewController *viewController = nil;
    NSArray * vcArray = reverse ? [[nav.viewControllers reverseObjectEnumerator] allObjects] : nav.viewControllers;
